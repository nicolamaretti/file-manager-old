...
<template>
    <AppLayout title="Manage Folder">
        ...
        <div class="py-8">
                ...
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
                    ...
                    <div class="mt-7">
                        <JetButton class="bg-red-400"
                                   @click="goBack('backend.file-manager.index')"
                        >
                            Cancel
                        </JetButton>
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 mb-8">
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
                    ...
                    <div class="pl-4">
                        <!--                        <div v-if="!isRoot">-->
                        <!--                            <p v-if="parent != null" class="underline text-blue-300" @click=(openFolder(parent.id))>Indietro</p>-->
                        <!--                            <p v-if="parent == null" class="underline text-blue-300" @click=(openFolder(null))>Indietro</p>-->
                        <!--                        </div>-->
                        ...
                        <div v-if="props.folder && !props.folders" class="inline-block">
                            <div class="whitespace-nowrap text-sm font-medium text-gray-900 inline-flex mt-1">
                                <input @click="selectFolder(props.folder.data)" type="radio" name="folder.name" id=folder.id
                                       value="folder" class="mt-1 cursor-pointer">

                                <IconFolder class="inline-block ml-3 my-auto cursor-pointer"
                                            @click="openFolder(props.folder.data.id)"
                                ></IconFolder>
                        ...
                        <div v-else-if="folders.data.length > 0" class="inline-block">
                            <div v-for="folder in folders.data">
                                <div class="whitespace-nowrap text-sm font-medium text-gray-900 inline-flex mt-1">
                                    <input @click="selectFolder(folder)" type="radio" name="folder.name" id=folder.id
                                           value="folder" class="mt-1 cursor-pointer">
        ...
        <JetConfirmationModal :show="confirmModal" @close="closeConfirmModal()">
            <template #title>
            ...
            <template #footer>
                <JetButton @click="closeConfirmModal()"
        ...
        <!-- modale errore -->
        <JetConfirmationModal :show="errorModal" @close="closeErrorModal">
            <template #title>
            <span class="text-center">
                Error
            </span>
            ...
            <template #footer>
                <JetButton @click="closeErrorModal"
                           class="bg-asred-200 text-white cursor-pointer mb-3 mr-2 px-6 py-2 rounded-sm hover:bg-gray-600">
                    <span>Ok</span>
                </JetButton>
            </template>
